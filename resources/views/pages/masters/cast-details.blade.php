        ...
        <div>
            <h2>Add Caste</h2>
                ...
                <div class="mt-5 flex space-x-2 items-center">
                    ...
                    <x-input placeholder="Enter Caste Name to Search" name="castName"/>

                    <button class="inline-block px-6 py-2.5 bg-indigo-600 text-white font-medium text-xs leading-tight my-2
                    uppercase rounded shadow-md hover:bg-indigo-700 hover:shadow-lg focus:bg-indigo-700 focus:shadow-lg focus:outline-none 
                   focus:ring-0 active:bg-indigo-800 active:shadow-lg transition duration-150 ease-in-out" onclick="search()">Search</button>
                </div>
                <div>
                    <table class="w-full">
                        <thead class="bg-white border-b">
...
<script>
    ...
    function search() {
        let cat = $("#searchcat").val();
        let cast = $("input[name=castName]").val();
